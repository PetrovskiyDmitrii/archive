...
class SelectConstructor {
  constructor(props, data = null) {
    ...
    if (this.config.init) {
      ...
      selectItems = Array.from(selectItems).filter(
        (item) => !item.closest(".flatpickr-calendar")
      );
  ...
  selectAction(selectItem) {
    this.selectsСlose(); // todo: maybe remove this?
    const originalSelect = this.getSelectElement(selectItem).originalSelect;
    const selectOptions = this.getSelectElement(
      selectItem,
      this.selectClasses.classSelectOptions
  ...
  getSelectTitleValue(selectItem, originalSelect) {
      ...
      selectTitleValue = this.getSelectedOptionsData(originalSelect)
        .elements.map(
          (option) =>
            ...
            }" data-value="${option.value}" class="_select-tag">${escape(
    ...
    selectTitleValue = escape(
      selectTitleValue.length
        ? selectTitleValue
        : originalSelect.dataset.placeholder
        ? originalSelect.dataset.placeholder
        : ""
    ...
    if (originalSelect.hasAttribute("data-search")) {
      ...
      return pseudoAttribute
        ? `<button type="button" class="${this.selectClasses.classSelectTitle}"><span class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"> 
<span class="select__icon-user">
                                 
                                                </span>
                                                <span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`
        : `<button type="button" class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`;
    }
  }
  ...
  getSelectElementContent(selectOption) {
    ...
    selectOptionContentHTML += selectOptionData
      ...
      : "";
    selectOptionContentHTML += escape(selectOption.textContent);
    selectOptionContentHTML += selectOptionData ? `</span>` : "";
    selectOptionContentHTML += selectOptionData ? `</span>` : "";
