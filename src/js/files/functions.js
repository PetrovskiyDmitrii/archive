  ...
  if (bodyLockStatus) {
    ...
    for (let index = 0; index < lock_padding.length; index++) {
      ...
      if (document.querySelector(".wrapper")) {
        ...
        el.style.paddingRight =
          window.innerWidth -
          document.querySelector(".admin").offsetWidth +
          "px";
      }
      if (document.querySelector(".wrapper")) {
...
export function spollers2() {
  const spollersArray = document.querySelectorAll("[data-spollers2]");
  if (spollersArray.length > 0) {
    // Отримання звичайних слойлерів
    const spollersRegular = Array.from(spollersArray).filter(function (
      item,
      ...
      return !item.dataset.spollers2.split(",")[0];
    });
    ...
    // Робота з контентом
    function initSpollerBody(spollersBlock, hideSpollerBody = true) {
      let spollerItems = spollersBlock.querySelectorAll("[data-spoller2-item]");
      if (spollerItems.length) {
        spollerItems.forEach((itemS) => {
          let spollerTitles = itemS.querySelectorAll("[data-spoller2]");
          if (spollerTitles.length) {
            spollerTitles = Array.from(spollerTitles).filter(
              (item) => item.closest("[data-spollers2]") === spollersBlock
            );
            const hideBlock = itemS.querySelector("[data-spoller-hide2]");
            spollerTitles.forEach((spollerTitle) => {
              if (hideSpollerBody) {
                spollerTitle.removeAttribute("tabindex");
                if (!spollerTitle.classList.contains("_spoller-active")) {
    ...
    function setSpollerAction(e) {
      const el = e.target;
      if (el.closest("[data-spoller2]")) {
        const spollerTitle = el.closest("[data-spoller2]");
        const spollersBlock = spollerTitle.closest("[data-spollers2]");
        const item = spollerTitle.closest("[data-spoller2-item]");
        const hideBlock = spollerTitle
          .closest("[data-spoller2-item]")
          .querySelector("[data-spoller-hide2]");
        const oneSpoller = false;
        const spollerSpeed = spollersBlock.dataset.spollersSpeed
          ? parseInt(spollersBlock.dataset.spollersSpeed)
          : 500;
        if (!spollersBlock.querySelectorAll("._slide").length) {
          ...
          _slideToggle(hideBlock, spollerSpeed);
        }
        e.preventDefault();
      }
    }
    ...
    function hideSpollersBody(spollersBlock) {
      const spollerActiveTitle = spollersBlock.querySelector(
        "[data-spoller2]._spoller-active"
      );
      const hideBlock = spollerActiveTitle
        .closest("[data-spoller2-item]")
        .querySelector("[data-spoller-hide2]");
      const spollerSpeed = spollersBlock.dataset.spollersSpeed
        ? parseInt(spollersBlock.dataset.spollersSpeed)
      ...
      if (
        ...
        spollerActiveTitle.classList.remove("_spoller-active");
        _slideUp(hideBlock, spollerSpeed);
      }
    }
  }
}
